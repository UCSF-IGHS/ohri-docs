...
A form is a collection of form fields classified through `pages` and `sections`. Forms are defined following a schema written in JSON. OHRI forms are leveraging the [AMPATH Forms schema](https://ampath-forms.vercel.app/core-concepts) with a few custom extensions. Below is a snippet of a basic form structure:
